...
//把octree中的节点进行的分组，分组的依据是节点之间的距离（阈值为R1）
//函数返回的是一个二维数组，每一列表示不同的组，每一行表示同一个组。
vector<vector<point3d>> extractFrontierPoints(const octomap::OcTree *octree) {
    ...
    for(octomap::OcTree::leaf_iterator n = octree->begin_leafs(octree->getTreeDepth()); n != octree->end_leafs(); ++n)
    {
      ...
      if(!cur_tree_2d->isNodeOccupied(*n))  //no occupied
      {
         x_cur = n.getX();
         y_cur = n.getY();
         z_cur = n.getZ();
	    ...
	    for (double y_cur_buf = y_cur - 0.1; y_cur_buf < y_cur + 0.15; y_cur_buf += octo_reso)
	    {
		  ...
		  continue;            
            ...
            if(frontier_true)// && num_free >5 )
            {
		      ...
		      if(distance < R1)    //R1 = 0.4 group length
		      {
...
{
  cout << "*** extractCandidateViewPoints running..." << endl;
    ...
    double distance_can;
            ...
            for(double yaw = 0; yaw < 2*PI; yaw += PI*2 / n)
	    {
                ...
                n_cur_3d = cur_tree->search(point3d(x, y, z));  //return pointer to node if found, NULL otherwise
...
vector<pair<point3d, point3d>> extractCandidateViewPoints(vector<vector<point3d>> frontier_groups, point3d sensor_orig ) 
{
//   cout << "*** extractCandidateViewPoints running; frontier_groups size = " << frontier_groups.size()
//     << ", sensor_orig.x = " << sensor_orig.x() << ", sensor_orig.y = " << sensor_orig.y()
//     << ", sensor_orig.z = " << sensor_orig.z() << endl;
  ...
  for(vector<vector<point3d>>::size_type u = 0; u < frontier_groups.size(); u++)
  {
    for(double yaw = 0; yaw < 2*PI; yaw += PI*2 / n)
    {
      x = frontier_groups[u][0].x() - R2 * cos(yaw);
      y = frontier_groups[u][0].y() - R2 * sin(yaw);
      //cout << "Candidate view point x = " << x << ", y = " << y << endl;
	...
	continue;
      ...
      if(sqrt(pow(x - sensor_orig.x(),2) + pow(y - sensor_orig.y(),2)) < 0.25)
      {
	//cout << "*** so close to sensor_orig! " << endl;
	candidate_valid = false;// delete candidates close to sensor_orig
	continue;
      }
      else
	...
	for(vector<vector<point3d>>::size_type n = 0; n < frontier_groups.size(); n++)
	{
	  ...
	  {
	    distance_can = sqrt(pow(x - frontier_groups[n][m].x(),2) + pow(y - frontier_groups[n][m].y(),2));
	    if(distance_can < R3)   //R3 = 0.25;  to other frontiers
	    {
	      //cout << "*** so close to R3!" << endl;
	      candidate_valid = false;        //delete candidates close to frontier
	      continue;
	    }
	  }
	...
	for (double x_buf = x - 0.3; x_buf < x + 0.3; x_buf += octo_reso) 
	  for (double y_buf = y - 0.3; y_buf < y + 0.3; y_buf += octo_reso)
	    for (double z_buf = sensor_orig.z()-0.2; z_buf <sensor_orig.z()+0.2; z_buf += octo_reso)
	    {
	      ...
	      if(!n_cur_3d)
	      {
		//cout << "*** 2, n_cur_3d is NULL" << endl;
		continue;
	      }
	      else if (cur_tree->isNodeOccupied(n_cur_3d))
	      {
		//cout << "*** n_cur_3d is occupied" << endl;
		candidate_valid = false;//delete candidates which have ccupied cubes around in 3D area
		continue;
	      }
	    }
...
{
    cout << "*** turtlebot_exploration_3d kinectCallbacks running..." << endl;
...
{
    cout << "*** turtlebot_exploration_3d laserscanCallbacks running..." << endl;
    pcl::PCLPointCloud2 cloud2;
    pcl_conversions::toPCL(*cloud2_msg, cloud2);
    PointCloud* cloud (new PointCloud);
    PointCloud* cloud_local (new PointCloud);
...
int main(int argc, char **argv) {
    cout << "*** turtlebot_exploration_3d main function running ..." << endl;
    ...
    //std::cout << "*** logfilename : " << logfilename << endl;
    ...
    //cout << "*** turtlebot_exploration_3d will subscribe and advertise ..." << endl;
    ...
    ros::Subscriber kinect_sub = nh.subscribe<sensor_msgs::PointCloud2>("/kinect2/qhd/points", 1, kinectCallbacks);// need to change##########
    ...
    //cout << "*** turtlebot_exploration_3d Update the initial location of the robot" << endl;
    ...
    for(int o =0; o < 6; o++)
    {
        ...
        while(!got_tf){
            try{
                ...
                tf_listener->lookupTransform("/map", "/kinect2_rgb_optical_frame", ros::Time(0), transform);// need to change tf of kinect###############

                cout << "*** kinect transform.getOrigin().x() = " << transform.getOrigin().x() << ", transform.getOrigin().y() = " << transform.getOrigin().y()
                    << ", transform.getOrigin().z() = " << transform.getOrigin().z() << endl;

                velo_orig = point3d(transform.getOrigin().x(), transform.getOrigin().y(), transform.getOrigin().z());                
                got_tf = true;
            }
            catch (tf::TransformException ex) {
                ROS_WARN("Update the initial location of the robot Wait for tf: Kinect frame"); 
            } 
        ...
        while(!got_tf){
            try{
                ...
                cout << "*** laser transform.getOrigin().x() = " << transform.getOrigin().x() << ", transform.getOrigin().y() = " << transform.getOrigin().y()
                    << ", transform.getOrigin().z() = " << transform.getOrigin().z() << endl;
                ...
                ROS_WARN("Update the initial location of the robot Wait for tf: LaserScan frame"); 
            } 
            ros::Duration(0.05).sleep();
        }
 
        // Take a Scan
        //cout << "*** turtlebot_exploration_3d ros will spin Once..." << endl;
        ros::spinOnce();
        ...
        ROS_WARN("Update the initial location of the robot Rotate 60 degrees");
    ...
    //cout << "*** Update the initial location of the robot over, loop 6 times" << endl;
    ...
    while(!got_tf){
        try{
            ...
            ROS_WARN("Update the pose of the robot Wait for tf: Kinect frame"); 
        }
        ros::Duration(0.05).sleep();
    }
    ...
    while(!got_tf){
        try{
            ...
            ROS_WARN("Update the pose of the robot Wait for tf: LaserScan frame"); 
        }
        ros::Duration(0.05).sleep();
    }
    ...
    //cout << "*** turtlebot_exploration_3d will run main loop ..." << endl;
    while (ros::ok())
    {
	...
	Frontier_points_cubelist.scale.x = octo_reso;        //octo_reso = 0.1  resolution of octomap,unit:meters
	...
	//show 3D octree
	/*
	for(octomap::OcTree::leaf_iterator n = cur_tree->begin_leafs(cur_tree->getTreeDepth()); 
	    n != cur_tree->end_leafs(); ++n)
	{
	...
	*/
	...
	Frontier_points_cubelist.points.clear();
	    ...
	    //###################################
	    frontier_groups=extractFrontierPoints( cur_tree_2d );
	    //###################################
            vector<pair<point3d, point3d>> candidates = extractCandidateViewPoints(frontier_groups, laser_orig);
            ROS_INFO("%lu candidates generated.", candidates.size());
        }
